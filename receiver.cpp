...
int Receiver::Messanger(SOCKET server, string Id) {
    ...
    do {
        ...
        while (strcmp(msg, "endfile") != 0) {
			...
			if (Recv(server, msg) == 1) {//total byte size
				int totalbytes = atoi(msg);
				FILE *fp = fopen(filepath.c_str(), "wb");
				int left = totalbytes;
				send(server, "000", MAX_BUFFER_SIZE, 0);// 성공적으로 받음
			}
			else {
				send(server,"001", MAX_BUFFER_SIZE, 0)// 실패
			}
			...
			if (Recv(server, msg) == 1) {//get filename.txt
				filepath = msg;
				send(server, "000", MAX_BUFFER_SIZE, 0);//성공적으로 받았고 다음 메세지 받을 준비 됨
			}
			else {
				send(server, "001", MAX_BUFFER_SIZE, 0);//실패
			}
